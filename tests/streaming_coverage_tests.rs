...
fn test_streaming_checkpoint_creation() {
    ...
    assert!(!checkpoint.config_hash.is_empty(), "Config hash should be set");
...
fn test_streaming_checkpoint_compatibility() {
    ...
    assert!(checkpoint.is_compatible(&config), "Checkpoint should be compatible with same config");
    
    // Test incompatibility with different config parameters
    let different_config = config.clone().with_max_lines(5000);
    assert!(!checkpoint.is_compatible(&different_config), "Checkpoint should be incompatible with different max_lines");
    
    let different_config2 = config.clone().with_include_tokens(true);
    assert!(!checkpoint.is_compatible(&different_config2), "Checkpoint should be incompatible with different token setting");
    
    let different_config3 = config.with_language(Some("Python".to_string()));
    assert!(!checkpoint.is_compatible(&different_config3), "Checkpoint should be incompatible with different language");
}
...
fn test_streaming_checkpoint_save_load() {
    ...
    let config = BatlessConfig::default();
    let original_checkpoint = StreamingCheckpoint::new(
        "test_file.txt".to_string(),
        25,
        1000,
        3,
    ...
    assert_eq!(loaded_checkpoint.line_number, original_checkpoint.line_number);
    assert_eq!(loaded_checkpoint.bytes_processed, original_checkpoint.bytes_processed);
    assert_eq!(loaded_checkpoint.chunk_number, original_checkpoint.chunk_number);
    assert_eq!(loaded_checkpoint.config_hash, original_checkpoint.config_hash);
    assert_eq!(loaded_checkpoint.schema_version, original_checkpoint.schema_version);
...
fn test_streaming_checkpoint_load_nonexistent() {
    ...
    assert!(result.is_err(), "Loading nonexistent checkpoint should fail");
}
...
fn test_streaming_checkpoint_load_invalid_json() {
    ...
    // Write invalid JSON
    std::fs::write(&checkpoint_path, "invalid json content")
...
fn test_streaming_processor_with_stdin() {
    ...
    if let Err(error) = result {
        assert!(error.to_string().contains("Resume/checkpoint") || 
...
fn test_streaming_processor_checkpoint_file_mismatch() {
    ...
    let result = batless::StreamingProcessor::process_streaming("current_file.txt", &config, Some(checkpoint));
    assert!(result.is_err(), "Mismatched file path should fail");
    
    if let Err(error) = result {
        assert!(error.to_string().contains("file path") || 
               error.to_string().contains("doesn't match"), 
...
fn test_streaming_processor_incompatible_checkpoint() {
    ...
    let result = batless::StreamingProcessor::process_streaming("test_file.txt", &config, Some(checkpoint));
...
fn test_streaming_get_schema() {
    ...
    assert!(schema["$schema"].is_string(), "Schema should have $schema field");
    assert!(schema["title"].is_string(), "Schema should have title field");
    assert!(schema["description"].is_string(), "Schema should have description field");
    assert_eq!(schema["type"], "object", "Schema type should be object");
}

#[test]
fn test_streaming_checkpoint_config_hash_changes() {
    let config1 = BatlessConfig::default()
        .with_max_lines(1000);
    
    let config2 = BatlessConfig::default()
        .with_max_lines(2000);
    ...
    assert_ne!(checkpoint1.config_hash, checkpoint2.config_hash, 
...
fn test_streaming_checkpoint_serialization() {
    ...
    assert!(json_str.contains("test_file.txt"), "JSON should contain file path");
    assert!(json_str.contains("42"), "JSON should contain line number");
    assert!(json_str.contains("1024"), "JSON should contain bytes processed");
    assert!(json_str.contains("5"), "JSON should contain chunk number");
    
    // Test deserialization from JSON
    let deserialized: StreamingCheckpoint = serde_json::from_str(&json_str)
