...
fn test_version_json_command() {
    ...
    assert!(stdout_str.contains("\"version\""), "Should contain version field");
    assert!(stdout_str.contains("\"git_hash\""), "Should contain git_hash field");
    assert!(stdout_str.contains("\"build_timestamp\""), "Should contain build_timestamp field");
    assert!(stdout_str.contains("\"authors\""), "Should contain authors field");
...
fn test_get_schema_file_info() {
    ...
    let stdout_str = String::from_utf8(output.stdout).expect("Valid UTF-8 output");
    assert!(stdout_str.contains("\"$schema\""), "Should contain schema field");
    
    // Verify it's valid JSON
    let _parsed: serde_json::Value = serde_json::from_str(&stdout_str)
        .expect("Output should be valid JSON schema");
}
...
fn test_get_schema_invalid() {
    ...
    assert!(stderr_str.contains("Unknown schema format"), "Should show error message");
    assert!(stderr_str.contains("Available schemas:"), "Should show available options");
}
...
fn test_configuration_wizard_help() {
    ...
    assert!(status_code == 0 || status_code != 0, "Command should execute");
}
...
fn test_edit_profile_nonexistent() {
    ...
    let status_code = output.status.code().unwrap_or(-1);
    assert!(status_code >= 0, "Command should handle nonexistent profile gracefully");
}
...
fn test_cli_with_max_lines_zero() {
    ...
    assert!(!output.status.success(), "Command should fail with max-lines=0");
    
    let stderr_str = String::from_utf8(output.stderr).expect("Valid UTF-8 output");
    assert!(stderr_str.contains("validation"), "Should show validation error");
}
...
fn test_cli_with_max_bytes_zero() {
    ...
    assert!(!output.status.success(), "Command should fail with max-bytes=0");
...
fn test_cli_with_invalid_theme() {
    ...
    assert!(!output.status.success(), "Command should fail with invalid theme");
    
    let stderr_str = String::from_utf8(output.stderr).expect("Valid UTF-8 output");
    assert!(stderr_str.contains("Theme not found") || stderr_str.contains("theme"), 
...
fn test_cli_with_invalid_language() {
    ...
    assert!(!output.status.success(), "Command should fail with invalid language");
    
    let stderr_str = String::from_utf8(output.stderr).expect("Valid UTF-8 output");
    assert!(stderr_str.contains("Language not found") || stderr_str.contains("language"), 
...
fn test_streaming_json_with_checkpoints() {
    ...
    let stdout_str = String::from_utf8(output.stdout).expect("Valid UTF-8 output");
    assert!(stdout_str.len() > 0, "Should produce streaming output");
}
...
fn test_ai_model_token_counting() {
    ...
    let stdout_str = String::from_utf8(output.stdout).expect("Valid UTF-8 output");
    assert!(stdout_str.contains("tokens"), "Should include token information");
}
...
fn test_fit_context_with_prompt_tokens() {
    ...
    assert!(stdout_str.len() > 0, "Should produce output");
}
...
fn test_custom_profile_nonexistent() {
    ...
    let output = run_batless_args(&[
        "--custom-profile", "nonexistent_profile.toml",
        temp_path
    ]);
    assert!(!output.status.success(), "Command should fail with nonexistent profile");
    
    let stderr_str = String::from_utf8(output.stderr).expect("Valid UTF-8 output");
    assert!(stderr_str.contains("profile") || stderr_str.contains("file"), 
...
fn test_error_message_formatting() {
    ...
    let stderr_str = String::from_utf8(output.stderr).expect("Valid UTF-8 output");
    assert!(stderr_str.contains("Error"), "Should contain 'Error' prefix");
    assert!(stderr_str.len() > 10, "Should contain meaningful error message");
}
...
fn test_completion_generation_all_shells() {
    ...
    assert!(output.status.success(), "Bash completion generation should succeed");
    let stdout = String::from_utf8(output.stdout).expect("Valid UTF-8");
    assert!(stdout.len() > 0, "Should generate bash completions");
    
    // Test zsh completion
    let output = run_batless_args(&["--generate-completions", "zsh"]);
    assert!(output.status.success(), "Zsh completion generation should succeed");
    let stdout = String::from_utf8(output.stdout).expect("Valid UTF-8");
    assert!(stdout.len() > 0, "Should generate zsh completions");
    
    // Test fish completion
    let output = run_batless_args(&["--generate-completions", "fish"]);
    assert!(output.status.success(), "Fish completion generation should succeed");
    let stdout = String::from_utf8(output.stdout).expect("Valid UTF-8");
